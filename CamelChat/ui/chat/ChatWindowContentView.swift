...
struct ChatWindowContentView: View {
  ...
  @ViewBuilder var list: some View {
    GeometryReader { geometry in
      HStack {
        List(viewModel.chatSourcesViewModel.sources, id: \.id, selection: $selectedSourceId) { source in
  ...
  var body: some View {
    NavigationSplitView {
      ...
      if let source = viewModel.chatSourcesViewModel.chatSourceViewModel(with: selectedSourceId) {
